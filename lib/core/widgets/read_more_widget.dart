...
class _ReadMoreWidgetState extends State<ReadMoreWidget> {
  ...
  Future<void> _translateText() async {
    ...
    if (widget.forceTranslation) {
      ...
      if (isHindiLocale && widget.text.trim().isNotEmpty) {
        ...
        try {
          ...
          final translated = await TranslationHelper.translateText(textWithPlaceholders, wordForWord: false, force: true);
    ...
    try {
      // Use text with placeholders so names are not translated
      final translated = await TranslationHelper.translateText(textWithPlaceholders);
