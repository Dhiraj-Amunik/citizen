...
class TranslationHelper {
  ...
  static Future<String> translateText(String? text, {bool wordForWord = false, bool force = false}) async {
    ...
    try {
      ...
      if (!force && ((isHindiLocale && isHindi) || (!isHindiLocale && !isHindi))) {
        return text;
      }
      ...
      if (force) {
        ...
        useAutoDetect = false;
        fromHindi = !isHindiLocale; // If locale is Hindi, translate from English (fromHindi=false)
      } else {
      ...
      final cacheKey = '$localeKey:${wordForWord ? 'word' : 'phrase'}:${force ? 'force' : 'auto'}:$text';
      ...
      try {
        ...
        final translated = wordForWord 
            ? await _translateWordForWord(text, fromHindi: fromHindi, useAutoDetect: useAutoDetect)
            : await _translateWithGoogle(text, fromHindi: fromHindi, useAutoDetect: useAutoDetect);
  ...
  static void _addToMemoryCache(String localeKey, String cacheKey, String translated) {
    if (!_memoryCache.containsKey(localeKey)) {
  ...
  static Future<void> clearCache() async {
    ...
    try {
      final prefs = await SharedPreferences.getInstance();
      final keys = prefs.getKeys().where((key) => key.startsWith('translation_cache_'));
  ...
  static Future<String> _translateWithGoogle(String text, {bool fromHindi = false, bool useAutoDetect = false}) async {
    try {
      final encodedText = Uri.encodeComponent(text);
      final locale = GeneralStream.instance.locale;
      final isHindiLocale = locale.languageCode == 'hi';
      // Use auto-detect for mixed-language messages, otherwise use specified source language
  ...
  /// [fromHindi] - If true, translates from Hindi to English, otherwise from English to Hindi
  /// [useAutoDetect] - If true, uses 'auto' as source language for better handling of mixed-language messages
  static Future<String> _translateWordForWord(String text, {bool fromHindi = false, bool useAutoDetect = false}) async {
    try {
      ...
      for (final word in words) {
        ...
        if (RegExp(r'^[^\w]+$').hasMatch(word) || RegExp(r'^\d+$').hasMatch(word)) {
          translatedWords.add(word);
          continue;
        }
        ...
        try {
          final translated = await _translateWithGoogle(word.trim(), fromHindi: fromHindi, useAutoDetect: useAutoDetect);
      ...
      return await _translateWithGoogle(text, fromHindi: fromHindi, useAutoDetect: useAutoDetect);
    }
  }
  ...
  static String transliterateToHindi(String input) {
    ...
    try {
      ...
      if (_nameCorrections.containsKey(lowerInput) || _nameCorrections.containsKey(input)) {
      ...
      if (lowerInput == 'deep' && (lowerTransliterated.contains('dape') || lowerTransliterated.contains('डेप'))) {
        return 'दीप';
      }
  ...
  static bool needsTranslation(String? text) {
    ...
    try {
      ...
      if (hasHindiChars) {
        ...
        isPrimarilyHindi = totalCharCount > 0 && (hindiCharCount / totalCharCount) >= 0.3;
      }
      ...
      return (isPrimarilyHindi && !isHindiLocale) || (!isPrimarilyHindi && isHindiLocale);
    } catch (e) {
      return false;
    }
  }
