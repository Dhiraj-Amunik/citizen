...
class _ChatContributeViewState extends State<ChatContributeView>
  ...
  Future<void> _fetchFullRequestDetailsIfNeeded() async {
    ...
    final hasDescription =
        _helpRequest.description != null &&
        _helpRequest.description!.trim().isNotEmpty;
    final hasTypeOfHelp = _helpRequest.typeOfHelp != null;

    // If we're missing critical fields, try to fetch them
    if ((!hasAddress || !hasDescription || !hasTypeOfHelp) &&
        _helpRequest.sId != null &&
        mounted) {
      try {
        ...
        try {
          ...
          if (myHelpRequestsVm.myWallOFHelpLists.isNotEmpty) {
            ...
            if (mounted && fullRequest.sId == _helpRequest.sId) {
              ...
              final hasBetterAddress =
                  _isValidAddress(fullRequest.address) && !hasAddress;
              final hasBetterDescription =
                  fullRequest.description != null &&
                  fullRequest.description!.trim().isNotEmpty &&
                  (!hasDescription ||
                      _helpRequest.description == null ||
                      _helpRequest.description!.trim().isEmpty);
              final hasBetterTypeOfHelp =
                  fullRequest.typeOfHelp != null && !hasTypeOfHelp;
              // Always update name from API if available (it's the correct name from request creator)
              final hasBetterName =
                  fullRequest.name != null && fullRequest.name!.isNotEmpty;

              if (hasBetterAddress ||
                  hasBetterDescription ||
                  hasBetterTypeOfHelp ||
                  hasBetterName) {
                setState(() {
                  _helpRequest = model.FinancialRequest(
                    ...
                    name:
                        fullRequest.name ??
                        _helpRequest.name, // Prioritize name from API
                    address: _getBestAddress(
                      _helpRequest.address,
                      fullRequest.address,
                    ),
                    description:
                        _helpRequest.description ?? fullRequest.description,
                    typeOfHelp:
                        _helpRequest.typeOfHelp ?? fullRequest.typeOfHelp,
          ...
          debugPrint(
            "MyHelpRequestsViewModel not available or request not found: $e",
          );
        }
        ...
        try {
          ...
          if (wallOfHelpVm.wallOFHelpLists.isNotEmpty) {
            ...
            if (mounted && fullRequest.sId == _helpRequest.sId) {
              // Check if we got better data
              final hasBetterAddress =
                  _isValidAddress(fullRequest.address) &&
                  !_isValidAddress(_helpRequest.address);
              final hasBetterDescription =
                  fullRequest.description != null &&
                  fullRequest.description!.trim().isNotEmpty &&
                  (_helpRequest.description == null ||
                      _helpRequest.description!.trim().isEmpty);
              final hasBetterTypeOfHelp =
                  fullRequest.typeOfHelp != null &&
                  _helpRequest.typeOfHelp == null;
              // Always update name from API if available (it's the correct name from request creator)
              final hasBetterName =
                  fullRequest.name != null && fullRequest.name!.isNotEmpty;
          ...
          debugPrint(
            "WallOfHelpViewModel not available or request not found: $e",
          );
        }

        // If still not found, try to fetch from API (search multiple pages if needed)
        if (mounted &&
            (!_isValidAddress(_helpRequest.address) || !hasTypeOfHelp)) {
      ...
      for (int page = 1; page <= 5; page++) {
        ...
        if (response.data?.responseCode == 200 && mounted) {
          ...
          try {
            ...
            if (mounted) {
              final hasBetterAddress =
                  _isValidAddress(request.address) &&
                  !_isValidAddress(_helpRequest.address);
              final hasBetterDescription =
                  ...
                  (_helpRequest.description == null ||
                      _helpRequest.description!.trim().isEmpty);
              final hasBetterTypeOfHelp =
                  request.typeOfHelp != null && _helpRequest.typeOfHelp == null;

              // Always update name from API if available (it's the correct name from request creator)
              final hasBetterName =
                  request.name != null && request.name!.isNotEmpty;
              if (hasBetterAddress ||
                  hasBetterDescription ||
                  hasBetterTypeOfHelp ||
                  hasBetterName) {
                setState(() {
                  _helpRequest = model.FinancialRequest(
                    ...
                    description:
                        _helpRequest.description ?? request.description,
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            getHelpDetails(
                              ...
                              desc:
                                  _helpRequest.typeOfHelp?.name?.capitalize() ??
                                  "Not available",
                            ),
                            getHelpDetails(
